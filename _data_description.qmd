...
You do not need to memorize every column name now, but this overview, can help you get familiar with the data. Think of this table as a trail map for the first few imports: it names the columns that will keep reappearing as you move from cells, to patterns, to genes.
...
| `replicate` | The donor/patient replicate label retained in the teaching dataset. | Use it when we need to know which samples came from the same individual that received interferon stimulation or not. |
| `UMAP1`, `UMAP2` | Two-dimensional coordinates used for visualization of similarity of samples. | Use them to explore the cell populations; do not treat distance on the plot as a direct biological measurement. |
